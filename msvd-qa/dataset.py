...
class MSVDQA(object):
    ...
    def __init__(self, train_batch_size, preprocess_dir):
        ...
        ###################
        # Train
        # # Create my own preprocessed qa_encode
        # self.bert_tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        # for i, sentence in enumerate(self.train_qa['question']):
        #     print(i)
        #     sent2indices = self.bert_tokenizer.tokenize(sentence)
        #     sent2indices = self.bert_tokenizer.convert_tokens_to_ids(sent2indices)
        #     self.train_qa['question_encode'][i] = unicode(",".join([str(word) for word in sent2indices]), "utf-8")
        #     ans2ind = self.bert_tokenizer.tokenize(self.train_qa['answer'][i])
        #     ans2ind = self.bert_tokenizer.convert_tokens_to_ids(ans2ind)
        #     if len(ans2ind) > 1:
        #         ans2ind = ans2ind[0]
        #     self.train_qa['answer_encode'][i] = np.int64(ans2ind)
        # with open(os.path.join(preprocess_dir, 'train_qa_encode_bert.json'), "w") as f:
        #     json.dump(self.train_qa, f)

        # # VAL
        # # Create my own preprocessed qa_encode
        # self.bert_tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        # for i, sentence in enumerate(self.val_qa['question']):
        #     print(i)
        #     sent2indices = self.bert_tokenizer.tokenize(sentence)
        #     sent2indices = self.bert_tokenizer.convert_tokens_to_ids(sent2indices)
        #     self.val_qa['question_encode'][i] = unicode(",".join([str(word) for word in sent2indices]), "utf-8")
        #     ans2ind = self.bert_tokenizer.tokenize(self.val_qa['answer'][i])
        #     ans2ind = self.bert_tokenizer.convert_tokens_to_ids(ans2ind)
        #     if len(ans2ind) > 1:
        #         ans2ind = ans2ind[0]
        #     self.val_qa['answer_encode'][i] = np.int64(ans2ind)
        # with open(os.path.join(preprocess_dir, 'val_qa_encode_bert.json'), "w") as f:
        #     json.dump(self.val_qa, f)

        # # Test
        # # Create my own preprocessed qa_encode
        # self.bert_tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        # for i, sentence in enumerate(self.test_qa['question']):
        #     print(i)
        #     sent2indices = self.bert_tokenizer.tokenize(sentence)
        #     sent2indices = self.bert_tokenizer.convert_tokens_to_ids(sent2indices)
        #     self.test_qa['question_encode'][i] = unicode(",".join([str(word) for word in sent2indices]), "utf-8")
        #     ans2ind = self.bert_tokenizer.tokenize(self.test_qa['answer'][i])
        #     ans2ind = self.bert_tokenizer.convert_tokens_to_ids(ans2ind)
        #     if len(ans2ind) > 1:
        #         ans2ind = ans2ind[0]
        #     self.test_qa['answer_encode'][i] = np.int64(ans2ind)
        # with open(os.path.join(preprocess_dir, 'test_qa_encode_bert.json'), "w") as f:
        #     json.dump(self.test_qa, f)
        ###################
