...
def main():
    ...
    parser.add_argument('--save_path', type=str, default=os.path.expanduser('~/kable_management/blp_paper/.results/egovqa/saved_models'),
    ...
    ######################################################################
    parser.add_argument('--jobname', type=str , default='egovqa-default',
    ...
    args.word_dim = 300
    args.vocab_num = 4000
    args.pretrained_embedding = os.path.expanduser('~/kable_management/data/EgoVQA/data/word_embedding.npy')
    ...
    args.preprocess_dir = os.path.expanduser("~/kable_management/data/EgoVQA/data")
    args.log = './logs'
    ...
    with open(os.path.expanduser("~/kable_management/data/EgoVQA/data/data_split.json"), 'r') as f:
        splits = json.load(f)
    ...
    dataset = dt.EgoVQA(args.batch_size, os.path.expanduser("~/kable_management/data/EgoVQA/data"), os.path.expanduser("~/kable_management/data/EgoVQA/data/feats"), video_cam_split)
    ...
    args.pretrain_model_path = os.path.expanduser("~/kable_management/blp_paper/.results/egovqa/pretrain_models") + 'model_%s_%s%s' % (args.image_feature_net,args.layer,args.memory_type)
    ...
    if args.memory_type=='_mrm2s':
        ...
        feat_channel *= 2
        rnn = TGIFBenchmark(args, feat_channel, feat_dim, text_embed_size, args.hidden_size,
                                 voc_len, num_layers, word_matrix, max_len=max_sequence_length)
        rnn = rnn.cuda()
        ...
        feat_channel *= 2
        rnn = LSTMEncDec(args, feat_channel, feat_dim, text_embed_size, args.hidden_size,
                                 voc_len, num_layers, word_matrix, max_len=max_sequence_length)
        rnn = rnn.cuda()
        ...
        rnn = CoMemory(args, feat_channel, feat_dim, text_embed_size, args.hidden_size,
                                 voc_len, num_layers, word_matrix, max_len=max_sequence_length)
        rnn = rnn.cuda()
    ...
    pretrain_path = os.path.join(os.path.expanduser("~/kable_management/blp_paper/.results/egovqa/pretrain_models"),
