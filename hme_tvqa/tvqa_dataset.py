...
class TVQADataset(Dataset):
    def __init__(self, opt, mode="train"):
        ...
        # Motion features
        self.c3d_h5 = h5py.File(os.path.expanduser("~/kable_management/data/tvqa/motion_features/fixed_tvqa_c3d_fc6_features.hdf5"), "r", driver=None)
    ...
    def __getitem__(self, index):
        ...
        # add motion (c3d) feature
        if self.vfeat_load:
            if self.with_ts:
                # lenny = cur_vid_feat.shape[0]
                # cur_c3d_feat = torch.ones(lenny, 4096)
                cur_c3d_feat = torch.from_numpy(self.c3d_h5[cur_vid_name][cur_start:cur_end])
            else:  # handled by vid_path
                cur_c3d_feat = torch.from_numpy(self.c3d_h5[cur_vid_name][:480])
            if self.normalize_v:
                cur_c3d_feat = nn.functional.normalize(cur_c3d_feat, p=2, dim=1)
        else:
            cur_c3d_feat = torch.zeros([2, 2])  # dummy placeholder
        items.append(cur_c3d_feat)
...
def pad_collate(data):
    ...
    #From jumperkables    
    def multia_pad_collate(sequences):
        # Texts
        q = [torch.LongTensor(s) for s in sequences[0]]
        a0 = [torch.LongTensor(s) for s in sequences[1]]
        a1 = [torch.LongTensor(s) for s in sequences[2]]
        a2 = [torch.LongTensor(s) for s in sequences[3]]
        a3 = [torch.LongTensor(s) for s in sequences[4]]
        a4 = [torch.LongTensor(s) for s in sequences[5]]
        # Lengths
        q_lengths = torch.LongTensor([len(seq) for seq in q])
        a0_lengths = torch.LongTensor([len(seq) for seq in a0])
        a1_lengths = torch.LongTensor([len(seq) for seq in a1])
        a2_lengths = torch.LongTensor([len(seq) for seq in a2])
        a3_lengths = torch.LongTensor([len(seq) for seq in a3])
        a4_lengths = torch.LongTensor([len(seq) for seq in a4])
        q_longest = max(q_lengths)
        a_longest = max(max(a0_lengths), max(a1_lengths), max(a2_lengths), max(a3_lengths), max(a4_lengths))
        longest = q_longest+a_longest
        # Create padded sequences
        padded_qa0 = torch.zeros(len(a0), longest).long()
        padded_qa1 = torch.zeros(len(a0), longest).long()
        padded_qa2 = torch.zeros(len(a0), longest).long()
        padded_qa3 = torch.zeros(len(a0), longest).long()
        padded_qa4 = torch.zeros(len(a0), longest).long()
        #Fill in questions the answers
        for idx, seq in enumerate(q):
            # Get the ends
            q_end = q_lengths[idx]
            a0_end = a0_lengths[idx]
            a1_end = a1_lengths[idx]
            a2_end = a2_lengths[idx]
            a3_end = a3_lengths[idx]
            a4_end = a4_lengths[idx]

            padded_qa0[idx, :q_end] = seq[:q_end]
            padded_qa0[idx, q_end:q_end+a0_end] = a0[idx][:a0_end]

            padded_qa1[idx, :q_end] = seq[:q_end]
            padded_qa1[idx, q_end:q_end+a1_end] = a1[idx][:a1_end]

            padded_qa2[idx, :q_end] = seq[:q_end]
            padded_qa2[idx, q_end:q_end+a2_end] = a2[idx][:a2_end]

            padded_qa3[idx, :q_end] = seq[:q_end]
            padded_qa3[idx, q_end:q_end+a3_end] = a3[idx][:a3_end]

            padded_qa4[idx, :q_end] = seq[:q_end]
            padded_qa4[idx, q_end:q_end+a4_end] = a4[idx][:a4_end]
        # Return
        candidates = torch.stack([padded_qa0, padded_qa1, padded_qa2, padded_qa3, padded_qa4], dim=1)
        candidates_lengths = torch.stack([
            q_lengths+a0_lengths,
            q_lengths+a1_lengths,
            q_lengths+a2_lengths,
            q_lengths+a3_lengths,
            q_lengths+a4_lengths
        ], dim=1)
        return candidates, candidates_lengths
    ...
    all_keys = text_keys + [label_key, qid_key, vid_name_key, vid_feat_key, c3d_feat_key, reg_feat_key, reg_topk_key]
    all_values = []
    for i, k in enumerate(all_keys):
        if k in text_keys:
            if k == "q":
                all_values.append( multia_pad_collate([ column_data[0], column_data[1], column_data[2], column_data[3],column_data[4],column_data[5]]) )
                pass
            else:
...
def preprocess_inputs(batched_data, max_sub_l, max_vcpt_l, max_vid_l, device="cuda:0"):
    ...
    max_len_dict = {"sub": max_sub_l, "vcpt": max_vcpt_l, "vid": max_vid_l, "c3d": max_vid_l, "reg":max_vid_l}
    text_keys = ["q", "a0", "a1", "a2", "a3", "a4", "sub", "vcpt"]
    label_key = "answer_idx"
    qid_key = "qid"
    vid_feat_key = "vid"
    ...
    model_in_list = []
    for k in text_keys + [vid_feat_key, c3d_feat_key, reg_feat_key, regtopk_feat_key]:
        #import ipdb; ipdb.set_trace()
...
if __name__ == "__main__":
    ...
    raise Exception("I've changed the __main__ function of this code, jumperkables")
